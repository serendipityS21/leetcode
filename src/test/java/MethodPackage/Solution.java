...
public class Solution {
    ...
    private void backtrack(List<String> res, StringBuffer sb, int dots,  int start) {
        if (dots == 4 && start == sb.length()){
        ...
        for (int i = start; i < sb.length(); i++) {
            ...
            int num = Integer.valueOf(sb.substring(start, i + 1));
            if (num >= 0 && num <= 255){
                if (sb.charAt(start) == '0' && (num != 0 || i != start)){
                    continue;
                }
                sb.insert(i + 1, '.');
                backtrack(res, sb, dots + 1, i + 2);
                sb.deleteCharAt(i + 1);
            }
        }
    }
    ...
    private void backtrack(List<List<String>> res, String s, ArrayList<String> path, int start) {
        if (start == s.length()){
            res.add(new ArrayList<>(path));
            return;
        }
